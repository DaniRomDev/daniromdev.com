...
Que buenos aquellos tiempos donde usabamos máquinas virtuales como Homestead o Valet para preparar nuestros entornos y empezar a trabajar con Laravel. Las cosas han cambiado y Docker ha pasado a ser una herramienta de contanerización demasiado útil como dejar pasarla de largo.
...
- Hay que tener en cuenta la arquitectura de tu sistema operativo donde vas a utilizar Docker _(por ejemplo si usas un Mac con el chip m1 solo podras usar imágenes que soporten arquitectura arm)_
...
Solo tengo una pega con ello, **necesitas composer instalado en tu sistema**, en mi opinión ya es una dependencia adicional que tengo que instalar en mi máquina para empezar a usar sail, ya que necesito instalarla primero como dependencia antes de usarla.

## Que infrastructura nos provee este template

En este template tendremos NGINX para servir nuestra aplicación tanto por HTTP como HTTPS, una base de datos MariaDB, un redis para almacenar nuestros jobs y consumirlos a través de colas, PHPmyAdmin para visualizar e interactuar con nuestras bases de datos y NGROK para compartir nuestro a través de una dirección web pública.

## Makefile es nuestro amigo

Este archivo te será familiar si usas algún sistema unix y cuando por ejemplo te bajas un paquete que no es oficial, en las instrucciones de instalación sale una instrucción parecida a: `chmod u+x ./install && ./make`.
...
Como podemos observar, ya no tenemos que tirar todos los comandos manualmente nosotros como usuarios, si no que directamente ejecutariamos `make o make build` ya que por defecto en estos archivos utiliza el primer comando definido como comportamiento por defecto si no le decimos cual.
...
Esto traducido sería,, ejecutame el comando **build** que inicializará en cadena los comandos: `install -> env -> docker/build-nc -> up `

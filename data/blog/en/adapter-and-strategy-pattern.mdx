...
Se podría aplicar un pequeño refactor y definir directamente la configuración del driver en el constructor de la clase y aquí simplemente instanciar pero las cosas podrían cambiar a corto/medio plazo y esta configuración podría tener otra fuente y no el framework de Laravel por ejemplo. Esto ya es según las necesidades del proyecto.

### Limpiando nuestro controlador que el nuevo patrón Adapter
...
use Illuminate\Http\Request;
Domain\Map\Contracts\LocatorContract as Locator

class GeoCodingController extends Controller
{
    public function index(Request $request, Locator $locator) {
        $map_position = $locator->findByIp($request->ip());
